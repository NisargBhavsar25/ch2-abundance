...
values=[]
def run_exp():
    weight=generate_random_array(len(elements),0,20)
    weight[l-1]=100-sum(weight[:-1])
    k=2
    for val in range(0,100,5):
        ...
        flux = phy_model(energy, weight, original_intensities)
        flux[flux<1e-5] = 0
        non_zero_indices = np.where(flux != 0)
        fys = flux[non_zero_indices]

    # fit a splined curve through the best flux
        flux = np.interp(energy[1:], energy[non_zero_indices], fys)
        fig, ax1 = plt.subplots(1,1, figsize=(12, 6))

        ax1.plot(energy[1:], original_intensities, label="Original")
        # ax1.plot(energy[1:], tflux, label="Original")
        ax1.plot(energy[1:], flux, label="Best")
        ax1.set_yscale('log')
        ax1.set_xlabel("Energy (keV)")
        ax1.set_ylabel("Intensity")
        ax1.legend()
        ax1.set_xlim([0, 10])  # Set x-axis limits
        print(f"{elements[k]} Value: ",sum(flux[0:250]))
        values.append(sum(flux[0:250]))
        
        plt.savefig(f"images/exp_0_{val}.jpg")
        plt.clf()

    # Create the plot
    plt.plot(range(len(values)), values, label="y = x^2", color="blue", marker="o")  # plot with markers on each point

    # Add labels and title
    plt.xlabel("X-axis")
    plt.ylabel("Y-axis")
    plt.title("Line Curve Plot")

    # Show legend
    plt.legend()

    # Display the plot
    plt.savefig(f"images/{elements[k]}")
