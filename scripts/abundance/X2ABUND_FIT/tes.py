...
base_path = '/home/manasj/Downloads/cla/miscellaneous/X2ABUND_LMODEL_V1/test/'
class_l1_data = os.path.join(base_path, 'ch2_cla_l1_20210827T210316000_20210827T210332000_1024.fits')
bkg_file = os.path.join(base_path, 'ch2_cla_l1_20210826T220355000_20210826T223335000_1024.fits')
scatter_atable = os.path.join(base_path, 'tbmodel_20210827T210316000_20210827T210332000.fits')
solar_model_file = os.path.join(base_path, 'modelop_20210827T210316000_20210827T210332000.txt')
response_path = base_path

static_par_file = os.path.join(base_path, '../static_par_localmodel.txt')
xspec_log_file = os.path.join(base_path, 'log_x2abund_test.txt')
xspec_xcm_file = os.path.join(base_path, 'xcm_x2abund_test.xcm')
plot_file = os.path.join(base_path, 'plots_x2abund_test.pdf')


ignore_erange = ["0.9","4.2"]
ignore_string = '0.0-' + ignore_erange[0] + ' ' + ignore_erange[1] + '-**'
# ignore_erange = ["0.9","2.0","3.0","4.2"]
# ignore_string = '0.0-' + ignore_erange[0] + ' ' + ignore_erange[1] +  '-' + ignore_erange[2] + ' ' + ignore_erange[3] + '-**'
# print('printing ignore_string', ignore_string)
...
# mo(4).link = '100 - (3+6+5+7+8+9+10)'
# mo(8).link = '100 - (3+4+5+6+7+9+10)'
Fit.nIterations = 100
Fit.perform()
Model.showList()
mo.show()
# Fit.goodness(sim=True)
...
('Time taken for the script to run is:', end - start)
